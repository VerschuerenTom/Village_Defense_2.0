...
public abstract class GameInstance extends BukkitRunnable {
    ...
    public void loadSignLines() {
        ...
        for (String s : config.getConfigurationSection("signs.format").getKeys(false)) {
            if(s.equalsIgnoreCase("WaitingForNewGame") || s.equalsIgnoreCase("FULL"))
                continue;
            ...
            signlines.put(GameState.fromString(s), new String[]{
                    ...
                    config.getString(path + "lines.1"),
                    config.getString(path + "lines.2"),
                    config.getString(path + "lines.3"),
                    config.getString(path + "lines.4")
            });

            FULLlines = new String[]{config.getString(path + "lines.1"),
                    config.getString(path + "lines.2"),
                    config.getString(path + "lines.3"),
                    config.getString(path + "lines.4")};
